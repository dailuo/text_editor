...
def format_called(filenameIn, *tuple_arg):
    ...
    #start
    for i in range(len(fileBeforeFormater)):
        if match3_1_list[i] != 'flag' and match3_2_list[i] != 'flag':
            cntList.append(cnt)
        elif match3_1_list[i] != 'flag':#if left bucket
            cntList.append(cnt)
            cnt += 1
    ...
    for i in range(len(fileBeforeFormater)):
        string = fileBeforeFormater[i]
        cntTmp = 0
        stringTmp = string.replace(' ', '')
        stringTmp = stringTmp.replace('\t','')
        if len(stringTmp) > 2:
            if stringTmp[0] == '/' and stringTmp[1] == '/':
                cntTmp = 1


        if string != '\n' and cntTmp == 0 and comment_list[i] == 0:
